...
import { Send, Instagram, MapPin, Mail, Phone } from "lucide-react";
...
export default function ContactSection() {
  ...
  const handleNewsletterSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const validatedData = insertNewsletterSubscriptionSchema.parse({ email: newsletterEmail });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          >
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" data-testid="contact-form">
              <div>
                <label htmlFor="name" className="block text-background/90 font-medium mb-2">
                  Full Name
                </label>
                <Input
                  id="name"
                  {...register("name")}
                  className="w-full px-4 py-3 bg-input border border-primary/30 rounded-lg text-background placeholder-background/50 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all pulse-glow"
              ...
              <div>
                <label htmlFor="email" className="block text-background/90 font-medium mb-2">
                  Email Address
                </label>
                <Input
                  id="email"
              ...
              <div>
                <label htmlFor="message" className="block text-background/90 font-medium mb-2">
                  Message
                </label>
                <Textarea
                  id="message"
                  {...register("message")}
                  rows={5}
                  className="w-full px-4 py-3 bg-input border border-primary/30 rounded-lg text-background placeholder-background/50 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all resize-none pulse-glow"
              ...
              <div className="flex space-x-6">
                {socialLinks.map((social, index) => (
                  <motion.a
                    key={social.label}
                    href={social.href}
                    className="text-3xl text-primary hover:text-background transition-colors"
            ...
            >
              ...
              <form onSubmit={handleNewsletterSubmit} className="flex gap-3">
                <Input
                  ...
                  className="flex-1 px-4 py-2 bg-input border border-primary/30 rounded text-background placeholder-background/50 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
                  required
